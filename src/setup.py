...
# creating empty daily reports collection
# daily reports document contains average, minimum, maximum, deviceid, date fields
# Reads devices.csv one line at a time, splits them into the data fields and inserts
with open(RELATIVE_CONFIG_PATH + DEVICE_COLLECTION + '.csv', 'r') as device_fh:
    for device_row in device_fh:
        device_row = device_row.rstrip()
...
# creating user access collection
# the document contains users with devices they have access to, each line represents one device only
# Reads useraccess.csv one line at a time, splits them into the data fields and inserts
with open(RELATIVE_CONFIG_PATH + USER_ACCESS + '.csv', 'r') as user_fh:
    for user_row in user_fh:
        user_row = user_row.rstrip()
        if user_row:
            (username, deviceid,access) = user_row.split(',')
        useraccess_data = {'username': username, 'device_id': deviceid, 'access': access}

        # This creates and return a pointer to the users collection
        user_collection = weather_dbh[USER_ACCESS]
        print(useraccess_data)
        # This inserts the data item as a document in the user collection
        user_collection.insert_one(useraccess_data)
